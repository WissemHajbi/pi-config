 ...
 * naturally.
 */
import { DynamicBorder, copyToClipboard, getMarkdownTheme, keyHint, type ExtensionAPI, type ExtensionCommandContext, type ExtensionContext, type Theme } from "@earendil-works/pi-coding-agent";
...
class TodoSelectorComponent extends Container implements Focusable {
	...
	private updateHints(): void {
		this.hintText.setText(
			this.theme.fg(
				"dim",
				"Type to search • ↑↓ select • Enter actions • Ctrl+N new • Ctrl+Shift+W work • Ctrl+Shift+R refine • Esc close",
			),
		);
	}
	...
	private updateList(): void {
		...
		if (this.filteredTodos.length === 0) {
			const message = this.allTodos.length === 0 ? "  No todos yet • Ctrl+N new" : "  No matching todos";
			this.listContainer.addChild(new Text(this.theme.fg("muted", message), 0, 0));
			return;
		}
	...
	handleInput(keyData: string): void {
		...
		if (matchesKey(keyData, Key.ctrl("n"))) {
			if (this.onCreateCallback) this.onCreateCallback();
			return;
		}
...
async function createTodoInteractive(
	todosDir: string,
	ctx: ExtensionCommandContext,
): Promise<TodoRecord | null | { error: string }> {
	if (!ctx.hasUI) {
		return { error: "Create todo requires interactive mode" };
	}

	await ensureTodosDir(todosDir);

	const title = (await ctx.ui.input("Todo title", "Short summary"))?.trim();
	if (!title) {
		return null;
	}

	const body = (await ctx.ui.input("Todo body", "Optional markdown notes")) ?? "";
	const tagsRaw = (await ctx.ui.input("Tags (comma-separated)", "bug, ui, docs")) ?? "";
	const statusChoice = await ctx.ui.select("Initial status", ["open", "closed"]);
	if (!statusChoice) {
		return null;
	}
	...
	const todo: TodoRecord = {
		...
		title,
		tags,
		status: statusChoice === "closed" ? "closed" : "open",
		...
		handler: async (args, ctx) => {
			...
			await ctx.ui.custom<void>((tui, theme, keybindings, done) => {
				...
				selector = new TodoSelectorComponent(
					...
					() => {
						void (async () => {
							const result = await createTodoInteractive(todosDir, ctx);
							if (!result) return;
							if ("error" in result) {
								ctx.ui.notify(result.error, "error");
								return;
