...
export default function (pi: ExtensionAPI) {
  try {
    require.resolve("@wissem_hajbi/pi-nointl/package.json");
    return;
  } catch {
    // Legacy fallback: continue registering this local shim.
  }

  const state = globalThis as typeof globalThis & {
    __nointl_extension_loaded__?: boolean;
  };
  if (state.__nointl_extension_loaded__) {
    return;
  }
  state.__nointl_extension_loaded__ = true;
  ...
  async function runScan(
    ...
    const runner = process.env.NOINTL_BIN || "nointl";
    const result = await pi.exec(runner, execArgs, {
      cwd,
      signal,
    });
  ...
  pi.registerCommand("nointl-scan", {
    ...
    handler: async (args, ctx) => {
      ...
      ctx.ui.notify(summary, run.result.issueCount === 0 ? "success" : "warning");
    },
  });
}
